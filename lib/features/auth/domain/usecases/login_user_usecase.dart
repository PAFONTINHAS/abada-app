...
class LoginUserUsecase {
  ...
  Future<Either<Failure, UserEntity>> call(
    ...
    String? paramsValidator = LoginUserUsecaseValidator.validate(
      userLoginParams,
    );

    if (paramsValidator != null)
      return Left(ValidationFailure(paramsValidator));

    final authenticateUser = await authRepository.authenticateUser(
      userLoginParams,
    );

    return authenticateUser.fold((failure) => Left(failure), (
      userCredential,
    ) async {
      if (userCredential.user == null)
        return Left(UnknownFailure("Usuário não encontrado"));
